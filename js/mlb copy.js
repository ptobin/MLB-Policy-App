...
function AutoRun() {
	...
	$("#channel-container").append ('<h1> AUTORUN IS RUNNING - Expired videos with restriction policy will be lifted </h1>');
...
$(document).ready(function() {
	...
	$('#autorun').change(function() {
		...
		if (AUTORUNMINS > 0) {
			...
			$("#channel-container").append ('<h1> AUTORUN IS RUNNING - Expired videos with restriction will be lifted </h1>');
			bAutoRun = true;
		...
		requestOptions = {
  		...
  		request.execute(function(response) {
			...
			if ((strTimeLeft == 'expired') && (restrictedvideo != 0)) { 
				...
				if (bAutoRun == false) {
					alert ("Video ID " + restrictedvideo + " has expired and is restricted");
				}
				else {
...
function createUploadMetaData(playlistId, pageToken) {
  ...
  request.execute(function(response) {
  	...
  	season_date.setDate(season_date.getDate()-DAYSBACK);
  		...
  		$('#message').html('Received ' + itemsReceived + ' items' + progress);
					...
					jQuery.each(policyListItems, function(index, item) {
						...
						if (item.name == 'Monetize in all countries') {
							...
							request.execute(function(response) {
								...
								$('#policy-update').append( '<br>video ' + v_id + ' updated at ' + response.timeCreated);
								//setTimeout(function() {
      							//	location.reload();
								//}, 2000);
